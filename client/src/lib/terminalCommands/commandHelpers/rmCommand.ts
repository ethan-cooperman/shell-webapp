...
const doRm: TerminalCommand = async (input: TerminalCommandInput) => {
  // make sure we have arguments
  if (input.argv.length !== 2) {
    return {
      output: `rm: expected 2 arguments`,
      isError: true,
    };
  }
  ...
  try {
    const response = await fetch(`${process.env.NEXT_PUBLIC_API_PATH}/api/rm`, {
      method: "DELETE",
      headers: {
